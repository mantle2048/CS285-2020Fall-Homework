    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        for i in range(self.num_target_updates * self.num_grad_steps_per_target_update):
            if i % self.num_grad_steps_per_target_update == 0:
                next_val_no = self.forward_np(next_ob_no)
                target_val_no = reward_n + self.gamma * next_val_no * (1 - terminal_n)
                target_val_no = ptu.from_numpy(target_val_no)

            val_no = self.forward(ptu.from_numpy(ob_no))
            self.optimizer.zero_grad()
            loss = self.loss(val_no, target_val_no)
            loss.backward()
            self.optimizer.step()
