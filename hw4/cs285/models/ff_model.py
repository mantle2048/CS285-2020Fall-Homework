...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        obs_normalized = normalize(obs_unnormalized, obs_mean, obs_std) # TODO/Done(Q1)
        acs_normalized = normalize(acs_unnormalized, acs_mean, acs_std) # TODO/Done(Q1)

        obs_normalized = ptu.from_numpy(obs_normalized)
        acs_normalized = ptu.from_numpy(acs_normalized)

        if(len(obs_normalized.shape)==1):
            obs_normalized = torch.unsqueeze(obs_normalized, dim=0)
        if(len(acs_normalized.shape)==1):
            acs_normalized = torch.unsqueeze(acs_normalized, dim=0)
        ...
        delta_pred_normalized = self.delta_network(concatenated_input).squeeze() # TODO/Done(Q1)
        next_obs_pred = obs_unnormalized + \
                unnormalize(ptu.to_numpy(delta_pred_normalized), delta_mean, delta_std) # TODO/Done(Q1)
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        )[0] # TODO/Done(Q1) get numpy array of the predicted next-states (s_t+1)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        :return:
        """
        target = ptu.from_numpy(normalize(
        ...
        prediction = self.forward(
            ...
            data_statistics['delta_mean'],
            data_statistics['delta_std']
        )[1] # TODO/Done(Q1) get numpy array of the predicted next-states (s_t+1)
